...
object Cmp {
  ...
  def apply[T, Δ](fn: (T, T) ⇒ ?[Δ])(implicit pos: sourcecode.FullName): Aux[T, Δ] = CanEq(fn)

  def ???[T, Δ](implicit pos: sourcecode.FullName) = Cmp[T, Δ] { (_, _) ⇒ Predef.??? }
   ...
   */
  case class Wrapper[T, D](cmp: Cmp.Aux[T, D]) { type Δ = D }
  object Wrapper {
